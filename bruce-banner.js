...
<head>
    ...
    <title>Bruce Banner - Fibonacci Universe</title>
    <style>
        body {
        ...
        #bruce-banner {
            /* Các style cơ bản được set thẳng trong JS, ở đây chỉ để dự phòng */
            width: 100%;
            max-width: 1100px;
        }
    </style>
...
<body>
    ...
    <div id="bruce-banner"></div>

    <script>
        // BỌC THÉP TOÀN BỘ CODE: Chờ HTML, CSS, Font load xong xuôi 100% mới chạy
        window.addEventListener("load", function() {
            console.log("🥋 Bruce Banner Init - Safe Mode (100% Loaded)...");
            ...
            let orionX = 0;
            let bruceX = -80;
            let walkTick = 0; // Biến đếm nhịp bước chân của Bruce
            ...
            function resizeCanvas() {
                ...
                // Cân bằng lại toạ độ Y
                bruceY = canvas.height - 75; // Điều chỉnh lại Y một chút cho vừa chiều cao mới của Bruce
            ...
            // Load 2 ảnh Bruce để làm animation đi bộ
            ...
            function resetScene(){
                console.log("♻️ Chu trình đã Reset!");
                ledOffset = 0;
                ledText = "";
                blackX = 40;
                blackY = -140;
            ...
            function drawStars() {
                ...
                // Logic đổi ảnh Bruce tạo hiệu ứng đi bộ
                let currentBruceImg = bruce1; // Mặc định đứng im

                if (bruce1.complete && bruce2.complete) {
                    if (!bruceAttached && !bruceKungfu) {
                        walkTick++;
                        // Thay đổi ảnh mỗi 15 frame (có thể tăng/giảm số 15 để chỉnh tốc độ bước)
                        if (Math.floor(walkTick / 15) % 2 === 0) {
                            currentBruceImg = bruce1;
                        } else {
                            currentBruceImg = bruce2;
                        }
                    }
                    ...
                    // Vẽ Bruce Lee (Tỉ lệ 45x75 để người bớt lùn, dáng thon gọn hơn)
                    if (currentBruceImg.naturalWidth > 0) {
                        ctx.drawImage(currentBruceImg, bruceX, bruceCurrentY, 45, 75);
                    }
                    ...
                    if (aiPanel) {
                        ...
                        // ĐÃ SỬA LỖI RESET: startX + textWidth là toạ độ kết thúc (đuôi) của dòng chữ. 
                        // Khi đuôi chữ bé hơn -50 (tức là chạy qua hẳn mép trái và thêm 1 khoảng an toàn) thì mới reset.
            ...
            drawStars();    
            console.log("⭐ Stars Animated (Safe Mode 100% Loaded)");      
        });
    </script>
</body>
</html>
