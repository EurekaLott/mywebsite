...
export async function renderVietlott(key) {
  ...
  try {
    ...
    document.getElementById('latestDate').innerHTML =
      ...
      `<button class="btn-copy-row" onclick='copyVietlottRow(${JSON.stringify(latest.numbers.map(pad2))}, this)'>📋 Copy</button>`;
    document.getElementById('latestBalls').innerHTML =
      latest.numbers.map(n => `<div class="ball">${pad2(n)}</div>`).join('');
